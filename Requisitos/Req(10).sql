...
--Asignación de la hora del dia y hora del sistema a la variable.
    ...
    CASE
        ...
        WHEN INSERTING THEN  
            v_operacion:=1;
            v_registros_antiguos:=null;
            v_registros_nuevos:=(:NEW.cod_articulo||'#'||:NEW.cod_proveedor||'#'||:NEW.cod_departamento||'#'||:NEW.precio_proveedor||'#'||:NEW.precio_venta||'#'||:NEW.nombre||'#'||:NEW.cantidad||'#'||:NEW.categoria);
       ...
       WHEN UPDATING THEN 
            v_operacion:=2;
            v_registros_antiguos:=(:OLD.cod_articulo||'#'||:OLD.cod_proveedor||'#'||:OLD.cod_departamento||'#'||:OLD.precio_proveedor||'#'||:OLD.precio_venta||'#'||:OLD.nombre||'#'||:OLD.cantidad||'#'||:OLD.categoria);
            v_registros_nuevos:=(:NEW.cod_articulo||'#'||:NEW.cod_proveedor||'#'||:NEW.cod_departamento||'#'||:NEW.precio_proveedor||'#'||:NEW.precio_venta||'#'||:NEW.nombre||'#'||:NEW.cantidad||'#'||:NEW.categoria);
